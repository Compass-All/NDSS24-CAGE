	...
	for (js = 0; js < kbdev->gpu_props.num_job_slots; js++) {
		...
		for (idx = 0; idx < SLOT_RB_SIZE; idx++) {
			...
			case KBASE_ATOM_GPU_RB_READY:
				...
				if(check_any_ccaext_katom() && stop_submit){// existing secure task and secure task is going on
					int pos=find_kctx_by_kctx_addr((uint64_t)katom[idx]->kctx);
					if (pos==-1){ //got ns task's kctx, because we cannot find it in the record
						break;
					}
					else{
						uint64_t cur_kctx_addr=total_kctx[pos].kctx_addr;
						...
						for (js = 0; js < kbdev->gpu_props.num_job_slots; js++) {
							...
							int pos1=-1;
							if(katom[0]){
								pos0=find_kctx_by_kctx_addr((uint64_t)katom[0]->kctx);
							}
							if(katom[1]){
								pos1=find_kctx_by_kctx_addr((uint64_t)katom[1]->kctx);
							}							
							if ((katom[0] && pos0!=-1 && katom[0]->gpu_rb_state == KBASE_ATOM_GPU_RB_SUBMITTED) || 
								(katom[1] && pos1!=-1 && katom[1]->gpu_rb_state == KBASE_ATOM_GPU_RB_SUBMITTED)) { 
									break_submit = 1; 
									break;
							}
						}
				...
				struct kbase_jd_atom *katom)
{
	...
	if (katom->kctx!=NULL){
		int pos=find_kctx_by_kctx_addr((uint64_t)katom->kctx);
		if (pos!=-1){
			stop_submit = 1;
		}
	}
