...
static uint64_t entry_set_ate(u64 *entry,
		...
		unsigned int level)
{
	u64 inputaddr=0;
	if (level == MIDGARD_MMU_BOTTOMLEVEL){
		inputaddr=as_phys_addr_t(phy) |get_mmu_flags(flags) |ENTRY_ACCESS_BIT | ENTRY_IS_ATE_L3;
	}
	else{
		inputaddr=as_phys_addr_t(phy) |get_mmu_flags(flags) |ENTRY_ACCESS_BIT | ENTRY_IS_ATE_L02;
	}
	page_table_entry_set(entry,inputaddr);
	return inputaddr;
}
