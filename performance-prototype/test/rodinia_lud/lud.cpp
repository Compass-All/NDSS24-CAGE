...
uint64_t app_uuid=0x55556666deadbeef;
struct dataset total_dataset[10];
uint64_t total_dataset_num=0;
...
int main ( int argc, char *argv[] ){
...
#ifdef CONFIDENTIAL
	char *my_backup_array_0 = (char*)malloc(matrix_dim*matrix_dim*sizeof(float));
	memcpy(my_backup_array_0, m, matrix_dim*matrix_dim*sizeof(float));

	total_dataset[0].set_addr=(uint64_t)my_backup_array_0;
  	total_dataset[0].set_size=(uint64_t)(matrix_dim*matrix_dim*sizeof(float));

	uint64_t infoarray_d_m[3][32];
		init_databuffer_infoarray(infoarray_d_m[0], app_uuid, 1,0, (uint64_t)(matrix_dim*matrix_dim*sizeof(float)),
      		CCAEXT_DATABUFFER_INPUT,(uint64_t)my_backup_array_0,(uint64_t)(matrix_dim*matrix_dim*sizeof(float)),0);
	init_databuffer_infoarray(infoarray_d_m[1], app_uuid, 1,0, (uint64_t)(matrix_dim*matrix_dim*sizeof(float)),
      	CCAEXT_DATABUFFER_NULL,0,0,0);
	init_databuffer_infoarray(infoarray_d_m[2], app_uuid, 1,0, (uint64_t)(matrix_dim*matrix_dim*sizeof(float)),
      	CCAEXT_DATABUFFER_OUTPUT,0,0,0);

	total_dataset_num=1;
	init_ccaext_region_and_pass_data(app_uuid, total_dataset_num,total_dataset);  
#endif
	...
	cl_mem d_m;
	d_m = clCreateNewBuffer(context, CL_MEM_READ_WRITE, matrix_dim*matrix_dim * sizeof(float), NULL, &err,app_uuid);
...
// 	  write_data_info(d_m, 1, 1);
// 	  printf("i = %d\n", i);
// #else
// 	  write_data_info(d_m, i==0, 0);
// #endif
//       dummy_buffer_end(cmd_queue);
	
#ifdef CONFIDENTIAL
	if (i==0){
		fill_description(d_m,infoarray_d_m[0]);
	}
	else {
		fill_description(d_m,infoarray_d_m[1]);
	}
	// clEnqueueWriteBuffer(cmd_queue, d_m,1,0,0x100,infoarray_d_m,0, NULL,NULL);
#endif
...
#ifdef CONFIDENTIAL
	// clEnqueueWriteBuffer(cmd_queue, d_m,1,0,0x100,infoarray_d_m,0, NULL,NULL);
	fill_description(d_m,infoarray_d_m[2]);
#endif
...
// 	// uint32_t ctx[8]; 
// 	memcpy(ctx, init_H, 32);
// 	sha256(ctx, m, matrix_dim*matrix_dim*sizeof(float));
// //	sha256_print_hash(ctx, "lud_result");
// #endif
...
#ifdef CONFIDENTIAL
	destroy_ccaext_region(app_uuid);
#endif
