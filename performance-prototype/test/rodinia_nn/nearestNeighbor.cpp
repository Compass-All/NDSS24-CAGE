...
char *data_folder;
char *app_uuid_str = "NearestNeighbor";
cl_context context=NULL;

uint64_t app_uuid=0xdeadbeefbeefdead;
void *my_backup_array;
...
int main(int argc, char *argv[]) {
  ...
  //Push data to the realm
    uint64_t infoarray_input[32]; 
    uint64_t infoarray_output[32]; 
#ifdef CONFIDENTIAL
  my_backup_array = (void*)malloc(sizeof(LatLong) * numRecords);
  memcpy(my_backup_array, (void*)&locations[0], sizeof(LatLong) * numRecords);
  struct dataset total_dataset[10];
  total_dataset[0].set_addr=(uint64_t)(my_backup_array);
  total_dataset[0].set_size=(uint64_t)(sizeof(LatLong) * numRecords);
    init_databuffer_infoarray(infoarray_input, app_uuid, 2,0, (uint64_t)(sizeof(LatLong) * numRecords),
      CCAEXT_DATABUFFER_INPUT,(uint64_t)(my_backup_array),(uint64_t)(sizeof(LatLong) * numRecords),0);
    init_databuffer_infoarray(infoarray_output, app_uuid, 2,1, (uint64_t)(sizeof(float) * numRecords),
      CCAEXT_DATABUFFER_OUTPUT,0,0,0);

  // Call the driver to create a realm and push data to the realm
  init_ccaext_region_and_pass_data(app_uuid, 1,total_dataset);
#endif
  ...
  recordDistances = OpenClFindNearestNeighbors(context,numRecords,locations,lat,lng,timing,app_uuid_str,infoarray_input,infoarray_output);
...
float *OpenClFindNearestNeighbors(
	...
	int timing, 
  char *app_uuid_str,
  uint64_t infoarray_input[],
  uint64_t infoarray_output[]
  ) {
    // 1. set up kernel
    cl_kernel NN_kernel;
    cl_int status;
    ...
    NN_kernel = clCreateKernel(
        cl_NN_program, app_uuid_str, &status);
    ...
    cl_event kernelEvent,readEvent;

#ifdef  TIMING
    gettimeofday(&tv_mem_alloc_start, NULL);
#endif

    d_locations = clCreateNewBuffer(context, CL_MEM_READ_WRITE,
        sizeof(LatLong) * numRecords, NULL, &error,app_uuid);
    d_distances = clCreateNewBuffer(context, CL_MEM_READ_WRITE,
        sizeof(float) * numRecords, NULL, &error,app_uuid);
...
#ifdef CONFIDENTIAL
    fill_description(d_distances,infoarray_output);
    fill_description(d_locations,infoarray_input);
#else
