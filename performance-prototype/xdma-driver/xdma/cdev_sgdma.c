...
{
//	printk("In char_sgdma_map_user_buf_to_sgl\n");
...
//	printk("file 0x%p, priv 0x%p, buf 0x%p,%llu, pos %llu, W %d, %s.\n",file, file->private_data, buf, (u64)count, (u64)*pos, write, engine->name);
	uint64_t par_result=0;

	//Double check here
	/*
    uint64_t *cmd_args=(uint64_t*)kzalloc(0x1000,GFP_KERNEL);
    memset(cmd_args,0x0,0x1000);
    copy_from_user(cmd_args,(void *)buf,0x1000);
	uint64_t start_addr=(uint64_t)cmd_args;
	uint64_t cnt=0;
	while(cnt<0x1000){
		uint64_t cur_addr=start_addr+cnt;
