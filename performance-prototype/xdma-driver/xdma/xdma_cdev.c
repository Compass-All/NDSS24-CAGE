...
static int create_sys_device(struct xdma_cdev *xcdev, enum cdev_type type)
{
	...
	if (!xcdev->sys_device) {
		...
		pr_info("device_create(%s) succeed\n", devnode_names[type]);
	}
...
void xpdev_destroy_interfaces(struct xdma_pci_dev *xpdev)
{
	...
	if (xpdev_flag_test(xpdev, XDF_CDEV_CAGE)) {
		rv = destroy_xcdev(&xpdev->cage_cdev);
		if (rv < 0)
			pr_err("Failed to destroy user cdev %d error 0x%x\n",
				i, rv);
	}
...
#endif
	printk("register cage\n");
	rv = create_xcdev(xpdev, &xpdev->cage_cdev, 0,
			 NULL, CHAR_CAGE);
	if (rv < 0) {
		pr_err("create xvc failed, %d.\n", rv);
		goto fail;
		...
		pr_info("create cage succeed\n");
		xpdev_flag_set(xpdev, XDF_CDEV_CAGE);
	}
